...
with tab2:
    ...
    if numeric_cols:
        ...
        try:
            ...
            # Sub-pestañas para organizar la información detallada devuelta por la función
            ...
            with sub_tab1:
                st.caption("Coeficientes de regresión ordenados por magnitud absoluta:")
                st.dataframe(model_results["coefficients"], width='stretch')
                
            with sub_tab2:
                st.caption("Comparativa de los primeros registros del conjunto de prueba (Test):")
                st.dataframe(model_results["predictions"], width='stretch')
                
                # Gráfico interactivo para visualizar Real vs Predicho
                import plotly.express as px
                fig_pred = px.scatter(
                    model_results["predictions"], x="real", y="predicho",
                    title="Valores Reales vs Predichos (Conjunto de Test)",
                    labels={"real": "Valor Real", "predicho": "Valor Predicho"},
                    template="plotly_white"
                )
                # Línea de referencia ideal y = x
                min_val = min(model_results["predictions"]["real"].min(), model_results["predictions"]["predicho"].min())
                max_val = max(model_results["predictions"]["real"].max(), model_results["predictions"]["predicho"].max())
                fig_pred.add_shape(
            ...
            with sub_tab3:
                st.caption(f"Variables independientes utilizadas como features ({len(model_results['features'])} en total):")
